...
export default function EntryRecordView({
  ...
  const fetchAll = useCallback(async () => {
    setLoading(true);
    try {
      const [recRes, secRes, attRes, lookRes] = await Promise.all([
        fetch(baseUrl),
      ...
      if (secRes.ok) {
        ...
        const fieldMap: Record<string, EntryField[]> = {};
        const relMap: Record<string, RelatedListItem[]> = {};

        await Promise.all(
          secs.map(async (sec: EntrySection) => {
            if (sec.sectionType === "fields") {
              const r = await fetch(
                `/api/lorekeeper/lorebooks/${lorebookId}/entry-types/${entryTypeId}/sections/${sec.id}/fields`
              );
              if (r.ok) fieldMap[sec.id] = (await r.json()).fields || [];
            } else {
              const r = await fetch(
        ...
        for (const item of items) {
          try {
            ...
            if (res.ok) {
              ...
              // Filter by lookup: find records that have a lookup pointing to current record via item.fieldId
              const related = data.records.filter((r: any) => {
                // We filter on client from lookups data
                return true;
              });
              const filtered: any[] = [];
              ...
              // For each record in that entry type, check if it has a lookup via item.fieldId pointing to recordId
              const allLookupRes = await fetch(
                `/api/lorekeeper/lorebooks/${lorebookId}/entry-types/${item.entryTypeId}/records/${recordId}/lookups?fieldId=${item.fieldId}`
              );
              // Actually we need to search for lookups where record1=recordId OR record2=recordId AND customFieldId=item.fieldId
              // Let's fetch lookups for current record filtered by fieldId
              ...
              if (myLookupRes.ok) {
                ...
                for (const lk of myLookups) {
                  ...
                  const otherRecord = data.records.find((r: any) => r.id === otherId);
                  if (otherRecord) {
                    filtered.push({
                      ...otherRecord,
      ...
      return (
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
          {fieldLookups.map((lk) => {
            ...
            // Find entry type of the other record (we don't have it here — show name only)
            return (
              <div
                key={lk.id}
                style={{
              ...
              >
                ...
                {canEdit && (
                  <ButtonIcon
                    ...
                    size="sm"
                    onClick={() => {
                      fetch(`${baseUrl}/lookups/${lk.id}`, { method: "DELETE" }).then(() => {
                        setLookups((l) => l.filter((x) => x.id !== lk.id));
                      });
                    }}
                  />
                )}
            ...
            {opts.map((opt) => {
              ...
              return (
                <span
                  key={opt.value}
                  onClick={() => {
                    if (isSelected) setFieldValue(field.id, selected.filter((v) => v !== opt.value));
                    else setFieldValue(field.id, [...selected, opt.value]);
                  }}
                  style={{
                    padding: "3px 10px",
                    borderRadius: 999,
              ...
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter" && e.currentTarget.value.trim()) {
                    setFieldValue(field.id, [...selected, e.currentTarget.value.trim()]);
    ...
    if (field.fieldType === "lookup") {
      ...
      return (
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          ...
          <Button variant="ghost" onClick={() => { setLookupModalField(field); setLookupSearch(""); }}>
            + Add lookup
          </Button>
        </div>
      );
  ...
  if (!record) {
    return <div style={{ padding: 20, color: "#64748b" }}>Entry not found.</div>;
  }
    ...
    <div style={{ height: "100%", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", gap: 4 }}>
          {canEdit && !editing && (
            <ButtonIcon name="edit" label="Edit entry" onClick={() => setEditing(true)} />
          )}
          {editing && (
            <>
          ...
          {canEdit && (
            <ButtonIcon name="trash" label="Delete entry" subvariant="danger" onClick={() => setShowDelete(true)} />
          )}
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px" }}>
        ...
        {/* Sections */}
        {sections
          .filter((s) => s.sectionType === "fields")
          .map((sec) => {
            const fields = fieldsBySection[sec.id] || [];
            if (fields.length === 0 && !editing) return null;
            return (
              <div key={sec.id} style={{ marginBottom: 24 }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 12 }}>
                  {sec.name}
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  {fields.map((field) => (
                    <div key={field.id}>
                      ...
                      {editing ? renderFieldEditor(field) : (
                        <div style={{ fontSize: 13, color: "#e2e8f0" }}>
                          {renderFieldValue(field, record.fieldData?.[field.id])}
                        </div>
                      )}
                    </div>
                  ))}
        ...
        {/* Related sections */}
        {sections.filter((s) => s.sectionType === "related_list").length > 0 && (
          <>
            <hr style={{ border: "none", borderTop: "1px solid #1e293b", margin: "0 0 20px" }} />
            <div style={{ fontSize: 16, fontWeight: 700, color: "#f1f5f9", marginBottom: 16 }}>Related</div>
            {sections.filter((s) => s.sectionType === "related_list").map((sec) => {
              const records = relatedRecords[sec.id] || [];
              return (
                <div key={sec.id} style={{ marginBottom: 20 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 10 }}>
                    {sec.name}
                  </div>
                  {records.length === 0 ? (
                      ...
                      {records.sort((a: any, b: any) => a.name.localeCompare(b.name)).map((r: any) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {relType && (
                              <span style={{ fontSize: 11, padding: "2px 6px", borderRadius: 4, background: relType.bgColor || "#334155", color: relType.fgColor || "#f1f5f9", flexShrink: 0 }}>
                                {relType.singularName}
                              </span>
                            )}
                          </div>
                        );
          ...
          {attachments.length === 0 ? (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(120px, 1fr))", gap: 10 }}>
              {attachments.map((att) => (
                <div
                  key={att.id}
                  style={{ background: "#1e293b", borderRadius: 8, overflow: "hidden", position: "relative" }}
                >
                  {/* Thumbnail / icon */}
                  <div
                    style={{ height: 80, display: "flex", alignItems: "center", justifyContent: "center", cursor: isPreviewSupported(att.filename) ? "pointer" : "default", background: "#0f172a" }}
...
function LookupModal({
  field, lorebookId, recordId, entryTypes, baseUrl, onClose, onAdded,
}: {
  field: EntryField;
  lorebookId: string;
  recordId: string;
  ...
  const handleAdd = async (targetRecord: any) => {
    ...
    if (res.ok) {
      ...
      const et = entryTypes.find((t) => t.id === targetRecord.entryTypeId);
        ...
        {(field.config?.aToB !== undefined || field.config?.bToA !== undefined) && (
          <div style={{ display: "flex", gap: 10 }}>
            <div style={{ flex: 1 }}>
              ...
              <input value={aToB} onChange={(e) => setAToB(e.target.value)} placeholder="e.g. father" style={{ width: "100%", background: "#1e293b", border: "1px solid #334155", borderRadius: 6, padding: "5px 8px", color: "#f1f5f9", fontSize: 13, outline: "none", boxSizing: "border-box" }} />
            </div>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 11, color: "#64748b", marginBottom: 4 }}>Relationship (B→A)</div>
              <input value={bToA} onChange={(e) => setBToA(e.target.value)} placeholder="e.g. son" style={{ width: "100%", background: "#1e293b", border: "1px solid #334155", borderRadius: 6, padding: "5px 8px", color: "#f1f5f9", fontSize: 13, outline: "none", boxSizing: "border-box" }} />
            </div>
          </div>
        )}
        <input
